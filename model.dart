...
class ImageTransferFacade {
  final _predictionModelFile = 'prediction_model.tflite';
  final _transformModelFile = 'transfer_model.tflite';
  ...
  Future<Uint8List> loadStyleImage(String styleImagePath) async {
    ...
    Uint8List bytes = (await NetworkAssetBundle(Uri.parse(styleImagePath))
            .load(styleImagePath))
        .buffer
        .asUint8List();
    return bytes;
  ...
  Future<Uint8List> loadoriginalImage(originalImagePath) async {
    //var originalImageByteData = await rootBundle.load(originalImagePath);
    //File imageFile = File(originalImagePath.path);

    Uint8List imageRaw = await originalImagePath.readAsBytes();

    return imageRaw;
  }

  Future transfer(Uint8List originData, Uint8List styleData) async {
